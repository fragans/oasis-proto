...
const cards = computed(() => [
  { label: 'Total Contacts', value: stats.value.contacts, icon: 'i-lucide-users', color: 'text-indigo-500', to: `${prefix.value}/audiences/contacts` },
  { label: 'Segments', value: stats.value.segments, icon: 'i-lucide-layers', color: 'text-violet-500', to: `${prefix.value}/audiences/segments` },
  { label: 'Attributes', value: stats.value.attributes, icon: 'i-lucide-tag', color: 'text-amber-500', to: `${prefix.value}/audiences/attributes` },
  { label: 'Event Types', value: stats.value.events, icon: 'i-lucide-zap', color: 'text-emerald-500', to: `${prefix.value}/audiences/events` }
])
</script>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div class="border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 bg-white dark:bg-zinc-900">
        ...
        <div class="space-y-2">
          ...
          />
          <UButton
            :to="`${prefix}/audiences/attributes`"
            variant="ghost"
            color="neutral"
            icon="i-lucide-tag"
