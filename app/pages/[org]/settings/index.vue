...
function handleAddOrg() {
  ...
  router.push(`/${org}/settings/organizations/create`)
}

function handleDelete(organization: OrganizationDetail) {
  if (!requireSuperAdmin()) return
...
async function toggleLive(organizationId: string, currentStatus: boolean | null) {
  ...
  try {
    ...
    if (response.success) {
      toast.add({
        title: 'Success',
        description: `Organization ${organizationId} is now ${!status ? 'LIVE' : 'OFFLINE'}`,
...
const deleteTarget = ref<OrganizationDetail | null>(null)
