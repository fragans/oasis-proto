...
function onCreated(journey: { id: string }) {
  router.push(`/${org.value}/journeys/${journey.id}`)
}
...
function getRowActions(journey: Journey) {
  return [
    { label: 'Open Builder', icon: 'i-lucide-pencil-ruler', onSelect() { router.push(`/${org.value}/journeys/${journey.id}`) } },
