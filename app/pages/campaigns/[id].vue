...
<template>
  <div class="max-w-4xl mx-auto space-y-6">
    ...
    <template v-else-if="campaign">
      ...
      <div class="flex items-start justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p v-if="campaign.objective" class="text-sm text-zinc-500 mt-1">
              {{ campaign.objective }}
            </p>
          </div>
        </div>
      ...
      <!-- Edit Form -->
      <div v-if="editing" class="border border-indigo-200 dark:border-indigo-800 rounded-xl bg-white dark:bg-zinc-900 p-6 space-y-4">
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-3 gap-4">
            <div>
              ...
              <USelect v-model="editForm.priority" :items="priorityOptions" />
            </div>
            <div>
              <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">Start Date</label>
              <UInput v-model="editForm.startDate" type="datetime-local" />
            </div>
            <div>
              <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">End Date</label>
              <UInput v-model="editForm.endDate" type="datetime-local" />
            </div>
          </div>
        </div>
        <div class="flex justify-end gap-2 pt-2">
          <UButton variant="ghost" color="neutral" label="Cancel" @click="editing = false" />
          <UButton label="Save Changes" color="primary" :loading="saving" @click="saveEdit" />
        </div>
      </div>

      <!-- Campaign Info -->
      <div v-else class="border border-zinc-200 dark:border-zinc-800 rounded-xl bg-white dark:bg-zinc-900 p-6">
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-6">
          <div>
            <p class="text-xs font-medium text-zinc-500 uppercase tracking-wider">Priority</p>
            <UBadge :color="(PRIORITY_COLORS[campaign.priority] as any)" variant="subtle" size="sm" class="mt-1.5 capitalize">
              {{ campaign.priority }}
            </UBadge>
          </div>
          <div>
            <p class="text-xs font-medium text-zinc-500 uppercase tracking-wider">Start Date</p>
            <p class="text-sm text-zinc-900 dark:text-white mt-1.5">{{ formatDate(campaign.startDate) }}</p>
          </div>
          <div>
            <p class="text-xs font-medium text-zinc-500 uppercase tracking-wider">End Date</p>
            <p class="text-sm text-zinc-900 dark:text-white mt-1.5">{{ formatDate(campaign.endDate) }}</p>
          </div>
          <div>
            <p class="text-xs font-medium text-zinc-500 uppercase tracking-wider">Days Remaining</p>
            <p class="text-sm text-zinc-900 dark:text-white mt-1.5">{{ campaign.daysRemaining ?? '—' }}</p>
          </div>
        </div>
        <div v-if="campaign.description" class="mt-6 pt-6 border-t border-zinc-100 dark:border-zinc-800">
          <p class="text-xs font-medium text-zinc-500 uppercase tracking-wider mb-1.5">Description</p>
          <p class="text-sm text-zinc-700 dark:text-zinc-300">{{ campaign.description }}</p>
        </div>
      </div>
      ...
      <div class="border border-zinc-200 dark:border-zinc-800 rounded-xl bg-white dark:bg-zinc-900 p-6">
        ...
        <div v-if="campaign.creatives && campaign.creatives.length > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div v-else class="text-center py-8">
          <UIcon name="i-lucide-image" class="w-10 h-10 mx-auto mb-2 text-zinc-300 dark:text-zinc-700" />
          <p class="text-sm text-zinc-500">No creatives uploaded yet</p>
        </div>
      </div>
